...
describe('OfficeBasePageComponent', () => {
  ...
  describe('onInitialisation', () => {
    it('should resolve state variables', () => {
      ...
      basePageComponent.commonPageState.routeNumber$.pipe(take(1)).subscribe((res: number) => expect(res).toEqual(3));
  ...
  describe('completeTestDelegated', () => {
    it('should dispatch store and then navigate to ' + 'rekey upload after the modal is dismissed', async () => {
      ...
      expect(basePageComponent.router.navigate).toHaveBeenCalledWith([DELEGATED_REKEY_UPLOAD_OUTCOME_PAGE], {
        replaceUrl: true,
      });
    });
  });
