...
describe('TestReportBasePageComponent', () => {
  ...
  describe('getTestRequirements', () => {
    it('should return correct value from getTestRequirementsCatB if the category is B', () => {
      expect(
        basePageComponent.getTestRequirements(
          {
    ...
    [TestCategory.C1E, TestCategory.C1, TestCategory.C].forEach((value) => {
      it(`should return correct value from getTestRequirementsCatC if category is ${value}`, () => {
        ...
        expect(basePageComponent.getTestRequirements({ testRequirements: { angledStart: true } }, value)).toEqual({
          angledStart: true,
        });
      });
    });
    [TestCategory.D1E, TestCategory.D1, TestCategory.DE, TestCategory.D].forEach((value) => {
  ...
  describe('hasManoeuvreBeenCompleted', () => {
    it('should call hasManoeuvreBeenCompletedCatADIPart2 if the category is ADI2', () => {
      expect(
        basePageComponent.hasManoeuvreBeenCompleted(
          {
            manoeuvres: [{ forwardPark: { selected: true } }, { reverseParkCarpark: { selected: true } }],
    ...
    it('should return null if the switch defaults', () => {
      expect(basePageComponent.hasManoeuvreBeenCompleted({}, TestCategory.EUA1M1)).toEqual(null);
    });
    ...
    it('should create LegalRequirementsModal if test report is invalid', async () => {
      ...
      expect(basePageComponent.modalController.create).toHaveBeenCalledWith({
        ...
        componentProps: {
          ...
          isDelegated: basePageComponent.isDelegatedTest,
        },
  ...
  describe('onModalDismiss', () => {
    it('should navigate to DEBRIEF_PAGE on CONTINUE event when test is not delegated', async () => {
      basePageComponent.isDelegatedTest = false;
    ...
    it('should navigate to OFFICE_PAGE on CONTINUE event when test is delegated', async () => {
      basePageComponent.isDelegatedTest = true;
