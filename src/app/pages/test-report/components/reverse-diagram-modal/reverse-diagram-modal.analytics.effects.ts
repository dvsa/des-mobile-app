...
export class ReverseDiagramModalAnalyticsEffects {
		...
		this.actions$.pipe(
			...
			switchMap(
				([, tests, applicationReference, candidateId, category]: [
					...
					this.analytics.setCurrentPage(formatAnalyticsText(AnalyticsScreenNames.REVERSE_DIAGRAM, tests));
					// GA4 Analytics
	...
	reverseDiagramLengthChanged$ = createEffect(() =>
		this.actions$.pipe(
			...
			concatMap((action) => of(action).pipe(withLatestFrom(this.store$.pipe(select(getTests))))),
			concatMap(
				([action, tests]: [ReturnType<typeof reverseDiagramActions.ReverseDiagramLengthChanged>, TestsModel]) => {
					// TODO - MES-9495 - remove old analytics
					this.analytics.logEvent(
						formatAnalyticsText(AnalyticsEventCategories.TEST_REPORT, tests),
	...
	reverseDiagramWidthChanged$ = createEffect(() =>
		this.actions$.pipe(
			...
			concatMap((action) => of(action).pipe(withLatestFrom(this.store$.pipe(select(getTests))))),
			concatMap(
				([action, tests]: [ReturnType<typeof reverseDiagramActions.ReverseDiagramWidthChanged>, TestsModel]) => {
					// TODO - MES-9495 - remove old analytics
					this.analytics.logEvent(
						formatAnalyticsText(AnalyticsEventCategories.TEST_REPORT, tests),
