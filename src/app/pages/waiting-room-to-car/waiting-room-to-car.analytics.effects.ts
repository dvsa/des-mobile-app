    ...
    this.actions$.pipe(
      ...
      switchMap(
        ([, tests, applicationReference, candidateId, category]: [
          ...
          this.analytics.setCurrentPage(formatAnalyticsText(AnalyticsScreenNames.WAITING_ROOM_TO_CAR, tests));
    ...
    this.actions$.pipe(
      ...
      switchMap(
        ([action, tests, category]: [ReturnType<typeof WaitingRoomToCarError>, TestsModel, CategoryCode, boolean]) => {
          ...
          this.analytics.addCustomDimension(AnalyticsDimensionIndices.TEST_CATEGORY, category);
          this.analytics.logError(`${AnalyticsErrorTypes.SUBMIT_FORM_ERROR} (${screenName})`, action.errorMessage);
    ...
    this.actions$.pipe(
      ...
      switchMap(
        ([action, tests, category]: [
          ReturnType<typeof WaitingRoomToCarValidationError>,
          TestsModel,
          CategoryCode,
          boolean,
        ]) => {
    ...
    this.actions$.pipe(
      ...
      switchMap(
        ([, tests, trainerRegistrationNumber]: [
          ReturnType<typeof TrainerRegistrationNumberChanged>,
          TestsModel,
          number,
          boolean,
        ]) => {
