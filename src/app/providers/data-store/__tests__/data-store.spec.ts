...
fdescribe('DataStoreProvider', () => {
  ...
  describe('getKeys', () => {
    ...
    it('should get `keys` from storage', async () => {
      spyOn(provider, 'isIos')
        .and
        .returnValue(true);
      storage.keys = jasmine.createSpy()
        .and
  ...
  describe('getItem', () => {
    ...
    it('should `get` data from storage', async () => {
      ...
      expect(storage.get)
        .toHaveBeenCalledWith(LocalStorageKey.LOGS);
    });
  });
  ...
  describe('setItem', () => {
    ...
    it('should call `set` with a key and a value', async () => {
      ...
      expect(storage.set)
        .toHaveBeenCalledWith(LocalStorageKey.LOGS, 'val1');
    });
  ...
  describe('removeItem', () => {
    ...
    it('should return an empty string if secureContainer is null', async () => {
      ...
      storage.remove = jasmine.createSpy()
        .and
        .returnValue(Promise.resolve());
  ...
  describe('migrateKey', () => {
    ...
    it('should call through to storage.set when they key is found', async () => {
      ...
      expect(storage.set)
        .toHaveBeenCalledWith(LocalStorageKey.CONFIG, 'this is the data we want');
    });
