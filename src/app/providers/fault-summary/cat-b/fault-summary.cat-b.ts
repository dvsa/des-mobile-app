...
export class FaultSummaryCatBHelper {
  ...
  private static getControlledStopFault(
    controlledStop: CatBUniqueTypes.ControlledStop,
  ...
  private static getVehicleCheckFaultsCatB(
    vehicleChecks: CatBUniqueTypes.VehicleChecks,
    faultType: CompetencyOutcome
  ): FaultSummary[] {
    const result: FaultSummary[] = [];
    ...
    const isValidDangerousFault: boolean =
      faultType === CompetencyOutcome.D && vehicleChecks.showMeQuestion.outcome === CompetencyOutcome.D;

    const isValidSeriousFault: boolean =
      faultType === CompetencyOutcome.S && vehicleChecks.showMeQuestion.outcome === CompetencyOutcome.S;
  ...
  private static getManoeuvreFaultsCatB(
