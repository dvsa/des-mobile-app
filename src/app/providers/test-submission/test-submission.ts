...
import { cloneDeep, isNull, isObject, unset } from 'lodash-es';
	...
	submitTest = (testToSubmit: TestToSubmit): Observable<HttpResponse<any> | HttpErrorResponse> => {
		...
		const cleanData = this.removeNullFieldsDeep(
			this.removePassCompletionWhenTestIsNotPass(
				this.isPartialSubmission(testToSubmit) ? this.removeFieldsForPartialData(deepClonedData) : deepClonedData
			)
		);
						...
						this.store$.dispatch(
							SaveLog({
								payload: this.logHelper.createLog(
									LogType.ERROR,
									`Submitting test with slot ID ${testToSubmit.slotId}`,
									err.message
								),
							})
						);
