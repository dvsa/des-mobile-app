  ...
  describe('singleAppMode', () => {
    ...
    describe('getDeviceType', () => {
      it('should return model', async () => {
        ...
        expect(deviceProvider.getDeviceType()).toBe('test');
      });
    });
    ...
    describe('getDescriptiveDeviceName', () => {
      ['iPad7,3', 'iPad7,4'].forEach((val) => {
        it(`should return FriendlyDeviceModel.iPAD_PRO_10_5_INCH if deviceModel is ${val}`, () => {
          spyOn(deviceProvider, 'getDeviceType').and.returnValue(val);
          expect(deviceProvider.getDescriptiveDeviceName()).toBe(FriendlyDeviceModel.iPAD_PRO_10_5_INCH);
        });
      });
      ['iPad11,6', 'iPad11,7'].forEach((val) => {
        it(`should return FriendlyDeviceModel.iPAD_8TH_GEN if deviceModel is ${val}`, () => {
          spyOn(deviceProvider, 'getDeviceType').and.returnValue(val);
          expect(deviceProvider.getDescriptiveDeviceName()).toBe(FriendlyDeviceModel.iPAD_8TH_GEN);
        });
      });
      it('should return FriendlyDeviceModel.iPAD_AIR_3RD_GEN if deviceModel is iPad11,4', () => {
        spyOn(deviceProvider, 'getDeviceType').and.returnValue('iPad11,4');
        expect(deviceProvider.getDescriptiveDeviceName()).toBe(FriendlyDeviceModel.iPAD_AIR_3RD_GEN);
      });
      it('should return FriendlyDeviceModel.iPAD_9TH_GEN if deviceModel is iPad12,2', () => {
        spyOn(deviceProvider, 'getDeviceType').and.returnValue('iPad12,2');
        expect(deviceProvider.getDescriptiveDeviceName()).toBe(FriendlyDeviceModel.iPAD_9TH_GEN);
      });
      it('should return switch value if the switch defaults', () => {
        spyOn(deviceProvider, 'getDeviceType').and.returnValue('test');
        expect(deviceProvider.getDescriptiveDeviceName()).toBe('test');
      });
    });
    ...
    describe('getUniqueDeviceId', () => {
      it('should return uuid', async () => {
        ...
        expect(deviceProvider.getUniqueDeviceId()).toBe('test');
      });
    });
    ...
    describe('is8thGenDevice', () => {
      ['iPad11,6', 'iPad11,7'].forEach((val) => {
        it(`should return true if deviceType is ${val}`, () => {
          expect(deviceProvider.is8thGenDevice(val)).toBe(true);
        });
      });
      it('should return fakse if deviceType does not fit the requirements', () => {
        expect(deviceProvider.is8thGenDevice('test')).toBe(false);
      });
    });
