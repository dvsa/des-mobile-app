...
describe('AuthenticationProvider', () => {
  ...
  describe('Provider', () => {
    ...
    it('determineAuthenticationMode() should set unauthenticated mode to true if offline', () => {
      ...
      expect(authenticationProvider.isInUnAuthenticatedMode())
        .toEqual(true);
    });
    ...
    it('isAuthenticated() should return true if in unauthenticated mode', async () => {
      spyOn(authenticationProvider, 'isInUnAuthenticatedMode')
        .and
        .returnValue(true);
    ...
    it('should call through to isAccessTokenAvailable,isAccessTokenExpired & refreshSession successfully', async () => {
      spyOn(authenticationProvider, 'isInUnAuthenticatedMode')
        .and
        .returnValue(false);
      spyOn(authenticationProvider.ionicAuth, 'isAccessTokenAvailable')
        .and
        .returnValue(Promise.resolve(true));
      spyOn(authenticationProvider.ionicAuth, 'isAccessTokenExpired')
        .and
        .returnValue(Promise.resolve(true));
      ...
      const isAuthenticated = await authenticationProvider.isAuthenticated();
      expect(authenticationProvider.ionicAuth.isAccessTokenAvailable)
        .toHaveBeenCalled();
      expect(authenticationProvider.ionicAuth.isAccessTokenExpired)
        .toHaveBeenCalled();
      expect(authenticationProvider.ionicAuth.refreshSession)
        .toHaveBeenCalled();
      expect(isAuthenticated)
        .toEqual(true);
    });
    ...
    * to check the functionality has not been broken if desired */
    xit('should throw at refresh and run through catch', async () => {
      ...
      spyOn(authenticationProvider.ionicAuth, 'refreshSession')
        .and
        .returnValue(Promise.reject(new Error('refresh, threw')));
      const isAuthenticated = await authenticationProvider.isAuthenticated();
      expect(authenticationProvider.ionicAuth.isAccessTokenAvailable)
        .toHaveBeenCalled();
      expect(authenticationProvider.ionicAuth.isAccessTokenExpired)
    ...
    it('getAuthenticationToken() should return a token', async () => {
      dataStoreProvider.getItem = jasmine.createSpy('setItem')
        .and
        .returnValue(Promise.resolve('"U0lMRU5UIEFZU05DIFRFU1QgVE9LRU4"'));
      spyOn(authenticationProvider.ionicAuth, 'isAuthenticated')
        .and
        .returnValue(Promise.resolve(true));
      spyOn(authenticationProvider.ionicAuth, 'getIdToken')
        .and
        .returnValue(Promise.resolve({
          exp: 1602686015366,
        }));
      ...
      expect(token)
        .toEqual('U0lMRU5UIEFZU05DIFRFU1QgVE9LRU4');
    });

    it('should call ionic login', async () => {
      spyOn(authenticationProvider.ionicAuth, 'login')
        .and
        .returnValue(Promise.resolve());
      await authenticationProvider.login();

      expect(authenticationProvider.ionicAuth.login)
        .toHaveBeenCalled();
    });
    ...
    describe('logout', () => {
      it('should logout successfully', async () => {
        ...
        expect(authenticationProvider.ionicAuth.logout)
          .toHaveBeenCalled();
        expect(dataStoreProvider.removeItem)
          .toHaveBeenCalledWith(Token.ID);
        expect(dataStoreProvider.removeItem)
          .toHaveBeenCalledWith(Token.ACCESS);
        expect(dataStoreProvider.removeItem)
          .toHaveBeenCalledWith(Token.REFRESH);
      });
      ...
      it('should clear the persisted tests when the configuration to do so is enabled', async () => {
        ...
        spyOn(appConfigProvider, 'getAppConfig')
          .and
          .returnValue(configWithPersistenceClearing);
        spyOn(authenticationProvider.ionicAuth, 'logout');
        await authenticationProvider.logout();
        expect(authenticationProvider.ionicAuth.logout)
          .toHaveBeenCalled();
    ...
    describe('expireTokens', () => {
      it('should call through to ionic auth expire() method', async () => {
        spyOn(authenticationProvider.ionicAuth, 'expire')
          .and
          .returnValue(Promise.resolve());
        await authenticationProvider.expireTokens();
        expect(authenticationProvider.ionicAuth.expire)
          .toHaveBeenCalled();
      });
    });
    ...
    describe('getAuthenticationToken', () => {
      beforeEach(() => {
        spyOn(authenticationProvider, 'hasValidToken')
          .and
          .returnValue(Promise.resolve(false));
      ...
      it('should expire the token if it is not valid before returning a new one', async () => {
        ...
        spyOn(dataStoreProvider, 'getItem')
          .and
          .returnValue(Promise.resolve(stringToken));
        const token = await authenticationProvider.getAuthenticationToken();
        expect(authenticationProvider.expireTokens)
          .toHaveBeenCalled();
        expect(token)
          .toEqual(JSON.parse(stringToken));
