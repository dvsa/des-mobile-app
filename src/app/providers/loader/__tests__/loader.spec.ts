...
describe('LoadingProvider', () => {
  ...
  describe('handleUILoading', () => {
    ...
    it('should not call create when isLoading is false', async () => {
      spyOn(loadingCtrl, 'getTop').and.returnValue(Promise.resolve(null as HTMLIonLoadingElement));
    ...
    it('should call getTop for each spinner that exists in order to dismiss them', async () => {
      spyOn(loadingCtrl, 'getTop').and.returnValues(
        Promise.resolve({ id: '1', dismiss: async () => { return true; } } as HTMLIonLoadingElement),
        Promise.resolve({ id: '2', dismiss: async () => { return true; } } as HTMLIonLoadingElement),
        Promise.resolve({ id: '3', dismiss: async () => { return false; } } as HTMLIonLoadingElement),
      );
      await service.handleUILoading(false);
      expect(loadingCtrl.create).not.toHaveBeenCalled();
      expect(loadingCtrl.getTop).toHaveBeenCalledTimes(3);
    });
  });
