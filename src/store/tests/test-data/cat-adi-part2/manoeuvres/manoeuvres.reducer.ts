...
export const manoeuvresCatADI2Reducer = createReducer(
	...
	on(
		...
		(state, { index }) =>
			[...state.slice(0, index), ...[{}], ...state.slice(index + 1)] as [
	...
	on(manoeuvresActions.RemoveManoeuvreFault, (state, { payload, index }) => {
		const { [payload.competency]: competencyToOmit, ...stateToPreserve } = state[index][payload.manoeuvre];
	...
	on(
		...
		(state, { fieldName, controlOrObservation, comment, index }) =>
			[
				...state.slice(0, index),
				...[
					{
