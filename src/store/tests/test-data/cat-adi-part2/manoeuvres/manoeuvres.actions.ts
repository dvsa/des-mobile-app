...
export const AddManoeuvreComment = createAction(
	...
	(fieldName: string, faultType: CompetencyOutcome, controlOrObservation: string, comment: string, index: number) => ({
		fieldName,
		faultType,
...
export const RemoveManoeuvreFault = createAction(
	...
	(payload: ManoeuvrePayload, index: number, faultLevel: CompetencyOutcome) => ({ payload, index, faultLevel })
);
