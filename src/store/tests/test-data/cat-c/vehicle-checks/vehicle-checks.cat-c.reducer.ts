...
export const vehicleChecksCatCReducer = createReducer(
	initialState,
	on(
		vehicleChecksCatCActionTypes.InitialiseVehicleChecks,
		(): CatCUniqueTypes.VehicleChecks => ({
			...generateInitialState(),
		})
	),
	on(
		vehicleChecksCatCActionTypes.ShowMeQuestionSelected,
		(state, { showMeQuestion, index }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			showMeQuestions: state.showMeQuestions.map((item, i) => (i === index ? showMeQuestion : item)),
		})
	),
	on(
		vehicleChecksCatCActionTypes.ShowMeQuestionOutcomeChanged,
		(state, { showMeQuestionOutcome, index }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			showMeQuestions: state.showMeQuestions.map((item, i) =>
				i === index
					? {
							...item,
	...
	on(
		vehicleChecksCatCActionTypes.TellMeQuestionSelected,
		(state, { tellMeQuestion, index }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			tellMeQuestions: state.tellMeQuestions.map((item, i) => (i === index ? tellMeQuestion : item)),
		})
	),
	on(
		vehicleChecksCatCActionTypes.TellMeQuestionOutcomeChanged,
		(state, { tellMeQuestionOutcome, index }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			tellMeQuestions: state.tellMeQuestions.map((item, i) =>
				i === index
					? {
							...item,
	...
	on(
		vehicleChecksCatCActionTypes.AddShowMeTellMeComment,
		(state, { comment }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			showMeTellMeComments: comment,
		})
	),
	on(
		vehicleChecksCatCActionTypes.VehicleChecksCompletedToggled,
		(state, { toggled }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			vehicleChecksCompleted: toggled,
		})
	),
	on(
		vehicleChecksCatCActionTypes.VehicleChecksDrivingFaultsNumberChanged,
		(state, { payload }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			showMeQuestions: [...payload],
		})
	),
	on(
		vehicleChecksCatCActionTypes.DropExtraVehicleChecks,
	...
	on(
		vehicleChecksCatCActionTypes.SetFullLicenceHeld,
		(state, { fullLicenceHeld }): CatCUniqueTypes.VehicleChecks => ({
			...state,
			fullLicenceHeld,
		})
	)
