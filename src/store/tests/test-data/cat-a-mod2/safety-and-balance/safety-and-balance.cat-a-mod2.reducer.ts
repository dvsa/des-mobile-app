...
export const safetyAndBalanceCatAMod2Reducer = createReducer(
	initialState,
	on(
		vehicleChecksCatAMod2ActionTypes.SafetyQuestionSelected,
		(state, { safetyQuestion, index }): SafetyAndBalanceQuestions => ({
			...state,
			safetyQuestions: state.safetyQuestions.map((item, i) => (i === index ? safetyQuestion : item)),
		})
	),
	on(
		vehicleChecksCatAMod2ActionTypes.SafetyQuestionOutcomeChanged,
		(state, { safetyQuestionOutcome, index }): SafetyAndBalanceQuestions => ({
			...state,
			safetyQuestions: state.safetyQuestions.map((item, i) =>
				i === index
					? {
							...item,
	...
	on(
		vehicleChecksCatAMod2ActionTypes.BalanceQuestionSelected,
		(state, { balanceQuestion, index }): SafetyAndBalanceQuestions => ({
			...state,
			balanceQuestions: state.balanceQuestions.map((item, i) => (i === index ? balanceQuestion : item)),
		})
	),
	on(
		vehicleChecksCatAMod2ActionTypes.BalanceQuestionOutcomeChanged,
		(state, { balanceQuestionOutcome, index }): SafetyAndBalanceQuestions => ({
			...state,
			balanceQuestions: state.balanceQuestions.map((item, i) =>
				i === index
					? {
							...item,
	...
	on(
		vehicleChecksCatAMod2ActionTypes.AddSafetyAndBalanceComment,
		(state, { comment }): SafetyAndBalanceQuestions => ({
			...state,
