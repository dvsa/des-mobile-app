...
export class TestsAnalyticsEffects {
  private formatter = (actionType: string) => actionType.replace('[TestsEffects] ', '').toLowerCase();
  ...
  sendCompletedTestsFailureEffect$ = createEffect(() =>
    this.actions$.pipe(
  ...
  sendPartialTestsFailureEffect$ = createEffect(() =>
    this.actions$.pipe(
  ...
  sendTestSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ofType(testActions.SendPartialTestSuccess, testActions.SendCompletedTestSuccess),
      concatMap((action) => of(action).pipe(withLatestFrom(this.store$.pipe(select(getTests))))),
      switchMap(
        ([action, tests]: [
          ReturnType<typeof testActions.SendPartialTestSuccess | typeof testActions.SendCompletedTestSuccess>,
          TestsModel,
        ]) => {
  ...
  testOutcomeChangedEffect$ = createEffect(() =>
    this.actions$.pipe(
  ...
  startTestAnalyticsEffect$ = createEffect(() =>
    this.actions$.pipe(
      ofType(StartTest),
      concatMap((action) =>
        of(action).pipe(
      ...
      switchMap(([action, applicationReference]) =>
        Promise.all([action, applicationReference, this.device.getDeviceInfo(), this.device.getBatteryInfo()])
      ),
      switchMap(([action, applicationReference, device, battery]) => {
        ...
        // GA4 Analytics
        const prefix = this.navigationStateProvider.isRekeySearch() ? 'RM_' : '';
