...
export const postTestDeclarationsReducer = createReducer(
  initialState,
  on(
    postTestDeclarationActions.ClearPostTestDeclarations,
    (): PostTestDeclarations => ({
      ...initialState,
    })
  ),
  on(
    postTestDeclarationActions.ToggleHealthDeclaration,
    (state): PostTestDeclarations => ({
      ...state,
      healthDeclarationAccepted: !state.healthDeclarationAccepted,
    })
  ),
  on(
    postTestDeclarationActions.HealthDeclarationAccepted,
    (state, { payload }): PostTestDeclarations => ({
      ...state,
      healthDeclarationAccepted: payload,
    })
  ),
  on(
    postTestDeclarationActions.ToggleReceiptDeclaration,
    (state): PostTestDeclarations => ({
      ...state,
  ...
  on(
    postTestDeclarationActions.PassCertificateNumberReceived,
    (state, { passCertNumberReceived }): PostTestDeclarations => ({
      ...state,
      passCertificateNumberReceived: passCertNumberReceived,
    })
  ),
  on(
    postTestDeclarationActions.SignatureDataChanged,
    (state, { signature }): PostTestDeclarations => ({
      ...state,
      postTestSignature: signature,
    })
  ),
  on(
    postTestDeclarationActions.SignatureDataCleared,
    (state): PostTestDeclarations => ({
      ...state,
      postTestSignature: '',
    })
