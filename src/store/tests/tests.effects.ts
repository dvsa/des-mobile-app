...
import { RekeySearchModel, getRekeySearchState } from '@pages/rekey-search/rekey-search.reducer';
...
export class TestsEffects {
	...
	persistTestsEffect$ = createEffect(
		() =>
			this.actions$.pipe(
	...
	loadPersistedTestsEffect$ = createEffect(() =>
		this.actions$.pipe(
			ofType(testActions.LoadPersistedTests),
			switchMap(() =>
				from(this.testPersistenceProvider.loadPersistedTests()).pipe(
	...
	startTestEffect$ = createEffect(() =>
		this.actions$.pipe(
			...
			concatMap((action) =>
				of(action).pipe(
					withLatestFrom(
						this.store$.pipe(select(getJournalState)),
						this.store$.pipe(select(getRekeySearchState)),
						this.store$.pipe(select(getDelegatedRekeySearchState)),
						this.store$.pipe(select(selectVersionNumber))
					)
				)
			),
			switchMap(
				([startTestAction, journal, rekeySearch, delegatedRekeySearch, appVersion]: [
					ReturnType<typeof StartTest>,
					JournalModel,
					RekeySearchModel,
					DelegatedRekeySearchModel,
					string,
					...
					const conductedLanguage: ConductedLanguage = testSlotAttributes.welshTest
						? Language.CYMRAEG
						: Language.ENGLISH;
					...
					if (
						...
						arrayOfActions.push(
							createPopulateVehicleDimensionsAction(startTestAction.category, slot.booking.application)
						);
					}
					...
					if (startTestAction.category === TestCategory.ADI3 || startTestAction.category === TestCategory.SC) {
	...
	activateTestEffect$ = createEffect(() =>
		this.actions$.pipe(
	...
	startPracticeTestEffect$ = createEffect(() =>
		this.actions$.pipe(
	...
	startSendingCompletedTestsEffect$ = createEffect(() =>
		this.actions$.pipe(
			ofType(testActions.StartSendingCompletedTests),
			switchMap(() => {
				return interval(this.appConfigProvider.getAppConfig()?.tests.autoSendInterval || TestsEffects.interval).pipe(
	...
	sendCompletedTestsEffect$ = createEffect(() =>
		this.actions$.pipe(
	...
	sendPartialTestSuccessEffect$ = createEffect(() =>
		this.actions$.pipe(
			ofType(testActions.SendPartialTestSuccess),
			switchMap((action) => {
				return [testStatusActions.SetTestStatusAutosaved(action.payload), testActions.PersistTests()];
			})
		)
	);

	sendCompletedTestsSuccessEffect$ = createEffect(() =>
		this.actions$.pipe(
			ofType(testActions.SendCompletedTestSuccess),
			switchMap((action) => {
				return [testStatusActions.SetTestStatusSubmitted(action.payload), testActions.PersistTests()];
			})
		)
	);
	...
	sendCurrentTestEffect$ = createEffect(() =>
		this.actions$.pipe(
	...
	sendCurrentTestSuccessEffect$ = createEffect(() =>
		this.actions$.pipe(
			...
			switchMap(([, slotId]: [testActions.TestActionsTypes, string]) => {
				return [testStatusActions.SetTestStatusSubmitted(slotId), testActions.PersistTests()];
			})
		)
	);
}
