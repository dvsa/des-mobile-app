...
export const passCompletionCatCReducer = createReducer(
	initialState,
	on(
		passCompletionActions.Code78Present,
		(state): CatCUniqueTypes.PassCompletion => ({
			...state,
			code78Present: true,
		})
	),
	on(
		passCompletionActions.Code78NotPresent,
		(state): CatCUniqueTypes.PassCompletion => ({
			...state,
			code78Present: false,
		})
	...
	on(
		...
		(state, { passCertificateNumber }): CatCUniqueTypes.PassCompletion => ({
			...state,
